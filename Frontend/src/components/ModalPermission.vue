      ...
      <div
        ...
        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
          <div class="sm:flex sm:items-start items-start">
            <template v-if="roleUpdate === 'admin'">
              <label class="block text-sm font-medium leading-6 text-gray-900"
                >Do you want to change your account to User?</label
              >
            </template>
            <template v-else-if="roleUpdate === 'user'">
              <label class="block text-sm font-medium leading-6 text-gray-900"
                >Do you want to change your account to Admin?</label
              >
            </template>
          </div>
        </div>
...
export default {
  ...
  methods: {
    async submitForm() {
      ...
      try {
        const endpoint = `/api/editRole/${this.accountIdUpdate}`;
        let formData = { role: "" };

        if (this.roleUpdate === "admin") {
          formData.role = "user";
        } else if (this.roleUpdate === "user") {
          formData.role = "admin";
        } else {
